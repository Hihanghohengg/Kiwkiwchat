...
### B. Pengujian Dinamis Backend API & WebSocket Signaling (6 Kasus Uji Minimum)
...
- **Total Kasus Uji**: 6 Kasus Uji (`BT-01` s/d `BT-06`)
- **Hasil**: **6/6 PASS (100%)**, **0 FAIL**
- **Cakupan Pengujian**:
...
| **T-11** | Akses API Lintas Domain (CORS) | CORS Whitelist dibatasi ke `ALLOWED_ORIGINS` | Static Code Review | **CODE_REVIEW_ONLY** |
...
11. **Backend CORS Configuration**: `CODE_REVIEW_ONLY` (`backend/main.py:114-119`)
...
2. **Penetapan T-11 s/d T-14**: T-12, T-13, T-14 diverifikasi secara dinamis melalui test suite lokal `tests/security/test_backend_websocket_security.py` (BT-01 s/d BT-06: 6/6 PASS), sedangkan T-11 CORS ditetapkan sebagai **CODE_REVIEW_ONLY** berbasis inspeksi `backend/main.py:ALLOWED_ORIGINS`.
...
2. **Lingkup Pemindaian OWASP ZAP**: Pemindaian dinamis ZAP hanya mencakup antarmuka web statis pada frontend Vercel (0 High, 1 Medium, 1 Low, 3 Info). Endpoint backend Render dan saluran WebSocket signaling diverifikasi secara lokal melalui test harness dinamis `BT-01` s/d `BT-06`.
